...
// Enhanced Reactive Features and Mini Game
document.addEventListener('DOMContentLoaded', function() {
    // Initialize all reactive features
    initReactiveFeatures();
    initMiniGame();
    initAchievementSystem();
});

// Reactive Features
function initReactiveFeatures() {
...
function createCursorParticle(x, y) {
    ...
    const colors = ['#6c63ff', '#ff6584', '#42b883', '#ffd166'];
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const children = entry.target.querySelectorAll('.reactive-hover');
...
function addElementHoverEffects() {
    const interactiveElements = document.querySelectorAll('.project-card, .activity-card, .experience-card');
    
    interactiveElements.forEach(el => {
        el.classList.add('interactive-element', 'reactive-hover');
...
function initMiniGame() {
    const gameLauncher = document.createElement('div');
    gameLauncher.className = 'game-launcher';
    gameLauncher.innerHTML = '🎮';
    gameLauncher.title = 'Play Mini Game';
    document.body.appendChild(gameLauncher);
    ...
    function createGameModal() {
        ...
        modal.innerHTML = `
            <div class="game-container">
                ...
                <div class="game-controls">
                    <button class="control-btn primary" id="restartGame">Restart</button>
                    <button class="control-btn secondary" id="closeGame">Close</button>
                </div>
            </div>
        `;
        ...
        modal.querySelector('#closeGame').addEventListener('click', closeGameModal);
        modal.querySelector('#restartGame').addEventListener('click', restartGame);
    ...
    function checkForMatch() {
        ...
        if (card1.dataset.symbol === card2.dataset.symbol) {
            ...
            createConfetti();
            showAchievement('Great memory! +10 points');
        } else {
            // No match
            setTimeout(() => {
    ...
    function updateGameDisplay() {
        ...
        if (movesEl) movesEl.textContent = gameData.moves;
        if (timeEl) timeEl.textContent = gameData.time + 's';
    }
    ...
    function endGame(victory) {
        ...
        if (victory) {
            gameData.score += Math.max(0, 100 - gameData.time);
            showAchievement('Game Completed! Final Score: ' + gameData.score);
            createConfetti(50);
        }
...
function initAchievementSystem() {
    window.showAchievement = function(message) {
        ...
        toast.innerHTML = `
            ...
            <span class="achievement-message">${message}</span>
        `;
...
function createConfetti(count = 30) {
    for (let i = 0; i < count; i++) {
        setTimeout(() => {
            ...
            // Random properties
            const colors = ['#6c63ff', '#ff6584', '#42b883', '#ffd166', '#ff6b6b'];
